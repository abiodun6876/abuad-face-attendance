...
class FaceRecognition {
      ...
      if (this.useTinyModel) {
        detection = await faceapi
          .detectSingleFace(img, new faceapi.TinyFaceDetectorOptions({
            inputSize: 160,
            scoreThreshold: 0.5 // Higher threshold to reduce false positive face detections
          }))
          .withFaceLandmarks()
          .withFaceDescriptor();
      } else {
  ...
  async findBestMatch(
    ...
    const MATCH_THRESHOLD = 0.75; // Increased from 0.65
  ...
  async matchFaceForAttendance(
    ...
    try {
      ...
      console.log(`Checking ${students?.length || 0} enrolled students for matches...`);
